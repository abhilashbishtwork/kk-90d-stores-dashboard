...
def build_discount_detail_query(store_names, start_date, end_date):
    """Gross sales + discount breakdown (merchant-funded vs aggregator-
    funded). Same non-cancelled scope as `build_revenue_query` so a
    discount rate is comparable to reported revenue — a cancelled
    order isn't real promotional spend."""
    stores_sql = _store_list_sql(store_names)
    return f"""
        SELECT
            toDate(o.created_at_ist) AS order_date,
            o.store_name AS store_name,
            o.channel AS channel,
            sum(o.sub_total_amount) AS gross_sales,
            sum(o.discount) AS discount,
            sum(o.aggregator_discount) AS aggregator_discount,
            sum(o.merchant_discount) AS merchant_discount,
            count(*) AS order_count
        FROM orders o
        LEFT JOIN (
            SELECT brand_id, order_id,
